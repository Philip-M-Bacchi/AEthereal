...
let encodeErrorCode = errAECoercionFail
let decodeErrorCode = errAECoercionFail
...
public class EncodeError: AutomationError {
    ...
    public init(object: Any, message: String? = nil, cause: Error? = nil) {
        self.object = object
        super.init(code: encodeErrorCode, message: message, cause: cause)
    }
    
    public override var message: String? {
        return "Can't encode unsupported \(type(of: self.object)) value:\n\n\t\(self.object)"
                + (self._message != nil ? "\n\n\(self._message!)" : "")
    }
}
...
public class CommandError: AutomationError { // raised whenever an application command fails
    ...
    public override var message: String? {
        return (self.reply?.forKeyword(AE4.Keywords.errorString)?.stringValue
                ?? self.reply?.forKeyword(AE4.OSAErrorKeywords.briefMessage)?.stringValue
    ...
    public var commandDescription: String {
        formatCommand(self.commandInfo, applicationObject: self.app.application)
    }
